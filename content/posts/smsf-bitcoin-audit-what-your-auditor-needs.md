...
I built [CertainKey](https://app.certainkey.dpinkerton.com) to make the second option easy. It's a self-service tool that walks you through the process: you provide your wallet descriptor, each key holder signs a challenge with their hardware wallet, and CertainKey generates an audit-ready PDF report with the blockchain-derived balance, cryptographic signatures, AUD valuation, and everything your auditor needs to sign off. The whole process takes minutes. No funds are moved, no sensitive data is retained after the report is generated, and every claim in the report can be independently verified.

If you want to see what the output looks like: [example report](https://app.certainkey.dpinkerton.com/example-report.pdf). There's also a [guide written specifically for auditors](https://app.certainkey.dpinkerton.com/auditor-guide.pdf) that explains the methodology and how to verify the findings independently — designed to sit alongside the report in your auditor's working papers.
